...
if (msg.content.startsWith("-stats")) {
  ...
  let u = msg.mentions.users.first() || msg.author;
  let d2 = await Perfil.findOne({ userID: u.id });
  if (!d2) return msg.reply("Sin perfil");
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      {
        name: "💪 Fuerza",
        value: `${d2.stats.fuerza} → ${tabla[d2.stats.fuerza].f} × ${m.f.toFixed(2)} = ${s.fuerza}`,
        inline: false
      },
      {
        name: "🛡️ Durabilidad",
        value: `${d2.stats.durabilidad} → ${tabla[d2.stats.durabilidad].d} × ${m.d.toFixed(2)} = ${s.durabilidad}`,
        inline: false
      },
      {
        name: "🔥 Vida",
        value: `${d2.stats.resistencia} → ${tabla[d2.stats.resistencia].res} × ${m.res.toFixed(2)} = ${s.resistencia}`,
        inline: false
      },
      {
        name: "⚡ Velocidad",
        value: `${d2.stats.velocidad} → ${tabla[d2.stats.velocidad].v} × ${m.v.toFixed(2)} = ${s.velocidad}`,
        inline: false
      },
      {
        name: "👁️ Reacción",
        value: `${d2.stats.reaccion} → ${tabla[d2.stats.reaccion].r} × ${m.r.toFixed(2)} = ${s.reaccion}`,
        inline: false
      },
      {
        name: "🔮 Energía Maldita",
        value: `${d2.stats.em} → ${tabla[d2.stats.em].em} × ${m.em.toFixed(2)} = ${s.em}`,
        inline: false
      },

      { name: "🎖️ Grado", value: d2.grado, inline: true }
    )
...
if(msg.content.startsWith("-perfil")){
  let u = msg.mentions.users.first() || msg.author;
  let d3 = await Perfil.findOne({ userID: u.id });
  if(!d3) return msg.reply("Sin perfil");

  const embed = new EmbedBuilder()
  .setTitle(`📜 Perfil`)
  .setDescription(`
Raza: ✦ ${d3.raza || "Sin raza"}
────────────────────
Clan: ✦ ${d3.clan || "Sin clan"}
Talento: ⚡ ${d3.talento || "sin talento"} 
Energía: 🔮 ${d3.energia || "sin energía"}
Potencial: 📈 ${d3.potencial || "sin potencial"}
Cuerpo: 💪 ${d3.cuerpo || "cuerpo normal"}
